...
def convert_uint16_to_float(depth, depth_range):
  """
  Convert depth values from uint16 to float32 using a per-frame depth_range.
  
  Args:
    depth: A tf.Tensor of shape (n, H, W, 1) with dtype tf.uint16.
            These values are assumed to be scaled in the [0, 65535] range.
    depth_range: A tf.Tensor of shape (n, 2) with dtype tf.float32.
                  Each row is of the form [min_depth, max_depth] for that frame.
  
  Returns:
    A tf.Tensor of shape (n, H, W, 1) with dtype tf.float32 where each depth value is
    converted to a float value in the range [min_depth, max_depth] corresponding to its frame.
  
  """
  # TODO: Replace placeholder with depth estimation conversion to float.
  depth_float = depth
  return depth_float
...
def convert_to_correct_format(bboxes_tensor: tf.Tensor, max_instances: int, bbox_threshold: float, valid_classes: tf.Tensor) -> tf.Tensor:
  """
  Converts bounding box tensor from (n, L, l) format to (n, max_instances, 4) format,
  ...
  Args:
      bboxes_tensor: A 3D tensor of shape (n, L, l), where the l values represent the bounding box specific information
  ...
  Returns:
      A tensor of shape (n, max_instances, 4), where the 4 values are [ymin, xmin, ymax, xmax].
  """
  # TODO: Convert 2D detections to shape (n, max_instances, 4), where the 4 values are [ymin, xmin, ymax, xmax]

  # TODO: Ensure the input tensor has the expected shape (replace with values for n,L,l)
  #tf.debugging.assert_shapes([(bboxes_tensor, (n, L, l))])

  # TODO: Extract relevant columns (allocate indexes, assuming normalized bounding box coordinates are available). Replace a,b,c, and d with corresponding indexes
  #ymin = bboxes_tensor[..., a]  # Shape (n, L)
  #ymax = bboxes_tensor[..., b]  # Shape (n, L)
  #xmin = bboxes_tensor[..., c]  # Shape (n, L)
  #xmax = bboxes_tensor[..., d]  # Shape (n, L)

  class_values = tf.cast(bboxes_tensor[..., -1], tf.int32)  # Shape (n, L)

  # Step 1: Create a valid mask based on class
  expanded_classes = tf.expand_dims(class_values, -1)  # Shape (n, L, l)
  matches = tf.equal(expanded_classes, valid_classes)  # Shape (n, L, num_valid_classes)
  valid_mask = tf.reduce_any(matches, axis=-1)  # Shape (n, L), True for valid rows

  # Step 2: Compute bounding box area and apply bbox_thresold
  width = (xmax - xmin) 
  height= (ymax - ymin)
  area = width * height # Shape (n, L)
  valid_area_mask = area >= bbox_threshold #True for bbs with area more than the thresh
  
  # Step 3 Combine the masks 
  combined_mask = tf.logical_and(valid_mask, valid_area_mask) # Shape (n,L)
  ...
  corners = tf.stack([ymin, xmin, ymax, xmax], axis=-1)  # Shape (6, L, 4)
  ...
  sorted_indices = tf.argsort(area, direction='DESCENDING', stable=True)  # shape: (6, L)
  # Gather the boxes according to these sorted indices. Use batch_dims=1.
  sorted_corners = tf.gather(corners, sorted_indices, axis=1, batch_dims=1)  # shape: (6, L, 4)
...
# TODO: implement preprocessing function below, specific to the available features of the autonomous driving dataset
...
class FromADD:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    # ---- TODO: Extract camera video and add to features dictionary ~5 lines----
    video = features[self.camera_key]["..."] 
    assert video.dtype == tf.uint8 # assert correct type
    video = tf.image.convert_image_dtype(video, tf.float32) # convert to float and normalize 
    video = tf.image.resize(video, [128, 192], method="bilinear") # resize
    ...
    # ---- TODO: Extract depth range and depth ~5 lines ----
    depth_range     = features[self.camera_key]["..."] # or set a constant tensor: tf.constant([3.0, 80.0], dtype=tf.float32)
    cam_depth_uint  = features[self.camera_key]["..."] 
    cam_depth_float = convert_uint16_to_float(cam_depth_uint, depth_range)
    cam_depth_float = tf.image.resize(cam_depth_float, [128, 192], method="nearest") # preserve depth discontinuities
    features_new[self.depth_key] = cam_depth_float

    # ---- TODO: Get bounding boxes ----
    cam_bboxes = features[self.camera_key]["..."]

    # ---- TODO (optional): Update allowed classes if neeeded to filter object detection classes
    allowed_classes  = tf.constant([0, 1, 2], dtype=tf.int32) # vehicles, peds, cyclists
    
    converted_bboxes = convert_to_correct_format(cam_bboxes, self.max_num_bboxes, self.bbox_threshold, allowed_classes)
