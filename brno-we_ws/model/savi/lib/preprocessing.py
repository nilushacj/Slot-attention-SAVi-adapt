...
def convert_to_corner_coordinates(bboxes_tensor: tf.Tensor, max_instances: int, bbox_threshold: float, valid_classes: tf.Tensor) -> tf.Tensor:
  """
  Converts bounding box tensor from (6, 20, 7) format to (6, max_instances, 4) format, 
  while filtering rows based on their class values.

  Args:
      bboxes_tensor: A 3D tensor of shape (6, 20, 7), where the 7 values represent normalized bounding box coordinates:
                    [frame index, x, y, width, height, confidence, class].
  ...
  Returns:
      A tensor of shape (6, max_instances, 4), where the 4 values are normalized [ymin, xmin, ymax, xmax].
  """

  # Ensure the input tensor has the expected shape
  tf.debugging.assert_shapes([(bboxes_tensor, (6, 20, 7))])

  # Extract relevant columns: x, y, width, height, and class
  x = bboxes_tensor[..., 1]  # Shape (6, 20)
  y = bboxes_tensor[..., 2]  # Shape (6, 20)
  width = bboxes_tensor[..., 3]  # Shape (6, 20)
  height = bboxes_tensor[..., 4]  # Shape (6, 20)
  class_values = tf.cast(bboxes_tensor[..., -1], tf.int32)  # Shape (6, 20)

  # Step 1: Create a valid mask based on class
  expanded_classes = tf.expand_dims(class_values, -1)  # Shape (6, 20, 1)
  matches = tf.equal(expanded_classes, valid_classes)  # Shape (6, 20, num_valid_classes)
  valid_mask = tf.reduce_any(matches, axis=-1)  # Shape (6, 20), True for valid rows

  # Step 2: Compute bounding box area and apply bbox_thresold
  area = width * height # Shape (6,20)
  valid_area_mask = area >= bbox_threshold #True for bbs with area more than the thresh
  
  # Step 3 Combine the masks 
  combined_mask = tf.logical_and(valid_mask, valid_area_mask) # Shape (6,20)

  # Step 4: Zero-pad invalid rows
  zero_tensor = tf.zeros_like(x)  # Create a zero tensor of the same shape
  x = tf.where(combined_mask, x, zero_tensor)  
  y = tf.where(combined_mask, y, zero_tensor)
  width = tf.where(combined_mask, width, zero_tensor)
  height = tf.where(combined_mask, height, zero_tensor)

  # Step 5: Convert to corner coordinates
  xmin = x - width / 2
  ymin = y - height / 2
  xmax = x + width / 2
  ymax = y + height / 2
  ...
  # Step 6 Stack and limit to max_instances
  corners = tf.stack([ymin, xmin, ymax, xmax], axis=-1)  # Shape (6, 20, 4)
  corners = corners[:, :max_instances, :]  # Shape (6, max_instances, 4)
...
class FromBrnoUrban:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    video = tf.image.convert_image_dtype(orig_video, tf.float32)  
    video = tf.image.resize(video, [128, 192], method="bilinear") 
    ...
    # ---- Extract depth ----
    depth_range     = tf.constant([3.0, 80.0], dtype=tf.float32)
    cam_depth_uint  = features[self.camera_key]["depth"] 
    cam_depth_float = convert_uint16_to_float(cam_depth_uint, depth_range[0], depth_range[1])
    ...
    allowed_classes  = tf.constant([0, 1, 2, 3, 5, 7], dtype=tf.int32)
    converted_bboxes = convert_to_corner_coordinates(cam_bboxes, self.max_num_bboxes, self.bbox_threshold, allowed_classes) 
