  ...
  metrics_update = train_metrics_cls.gather_from_model_output(
      ...
      **loss_aux,
      ...
      ground_truth_segmentations=utils.remove_singleton_dim(
        preds["outputs"].get("segmentations")), #NOTE: mere placeholder as model does not use ground truth segmentations
...
def train_and_evaluate(config: ml_collections.ConfigDict,
  ...
  # ---- a key to control randomness (i.e. generate same random seed for reproducability) ----
  ...
  # Input pipeline.
  # ---- ensure same random numbers (2x1 tuple of keys) everytime ----
  ...
  if config.get("seed_data", True):  # Default to seeding data if not specified.
    data_rng = jax.random.fold_in(data_rng, jax.process_index()) # gives another 2x1 tuple of two keys
  else:
    data_rng = None

  train_ds, eval_ds = input_pipeline.create_datasets(config, data_rng)
  logging.info(f" **************** train_ds:{train_ds} *********************")
  logging.info(f" **************** eval_ds:{eval_ds}   *********************")
