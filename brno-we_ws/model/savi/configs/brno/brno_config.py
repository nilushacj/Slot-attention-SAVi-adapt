...
def get_config():
    ...
    config.data = ml_collections.ConfigDict({
        "name":"brno_video_winter/video6:1.0.0",
        "data_dir":"/PATH/TO/MANUAL/DIR", #TODO: set path to manual dir
        "shuffle_buffer_size": config.batch_size * 8 
    })
    
    config.camera_select = 'cam_left_front'
    config.max_instances = 10
    config.bbox_thresh = 0.005

    config.num_slots = config.max_instances + 1
    config.logging_min_n_colors = config.max_instances


    config.preproc_train = [
        f"from_brno_urban(camera_key='{config.camera_select}', max_num_bboxes={config.max_instances}, bbox_threshold={config.bbox_thresh})",
        "random_resized_crop(height=128, width=192, min_object_covered=0.2)",
